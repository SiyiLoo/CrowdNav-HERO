...
def main(args):
    ...
    if policy_config.name == 'model_predictive_rl':
        ...
        trainer = MPRLTrainer(value_estimator=model, state_predictor=policy.state_predictor, map_predictor=policy.map_predictor, memory=memory, ped_memory=ped_memory, ped_memory_batch=ped_memory_batch, device=device, policy=policy, writer=writer, batch_size=batch_size, optimizer_str=optimizer, human_num=env.human_num,
    ...
    explorer = Explorer(env, robot, device, writer, memory, ped_memory, cur_memory, ped_memory_batch=ped_memory_batch,
