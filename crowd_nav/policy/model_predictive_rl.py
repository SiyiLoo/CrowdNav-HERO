...
class ModelPredictiveRL(Policy):
    ...
    def load_state_dict(self, state_dict):
        if self.state_predictor.trainable and self.map_predictor.trainable:
            if self.lsy:
                ...
                self.state_predictor.human_motion_predictor_RGL.load_state_dict(state_dict['human_motion_predictor_RGL'])
    ...
    def predict(self, state_last, state, cur_memory, local_map = None):
        ...
        if self.phase == 'train' and probability < self.epsilon:
            ...
            for action in action_space_clipped:
                ...
                max_next_return, max_next_traj = self.V_planning(next_state, next_local_map, self.planning_depth, self.planning_width)
                #根据获得的动作来计算收益，因为奖励和是否发生碰撞，是否在安全区域里面有关，这都和行人的动作有关
    ...
    def V_planning(self, state, local_map, depth, width):
        ...
        current_state_value = self.value_estimator(state, local_map)
        if depth == 1:
    ...
    def estimate_reward(self, state_last, state, local_map, action):
        ...
        if collision:
            ...
            reward = 1 / dis_reaching_goal - 0.01
            #reward = - 0.01
        return reward
